...
> 注：PSR、3DCNN、X2CT-GAN及原论文Diff2CT的指标引自文献[4] Table 1（私有腰椎数据集LumbarV，268例CT，128³分辨率，1000 epoch）；X-CTRSNet、X2CT-GAN、DiffuX2CT的指标引自文献[6] Table 1（公开脊柱数据集CTSpine1K，128³分辨率）；X2CT-CNN、LDM、CLS-DM的指标引自文献[8] Table 1（CTSpine1K，128³分辨率，双视角）。本复现结果为VerSe 2019数据集。v2版本为RTX 4080 SUPER上128³体积、80例数据、300 epoch训练，修复了投影损失并加入梯度检查点；v1版本为96³体积、30例数据、200 epoch；CPU基线为64³体积、10 epoch。指标与原论文差距主要因训练数据量有限及训练轮次差异。
...
- 训练300 epoch后，噪声预测损失从 1.10 降至 0.005，最佳验证集损失：0.0024
- 评测结果：MAE 0.5677±0.0404，PSNR 7.86±1.48，SSIM 0.1428±0.0300，FID 170.41
- 相比v1版本：PSNR提升46%（5.36→7.86），SSIM提升168%（0.053→0.143）

**v1 GPU训练：**
- 训练200 epoch后，噪声预测损失从 1.10 降至 0.005
- 最佳验证集损失：0.0011
- 评测结果：MAE 0.5919±0.0156，PSNR 5.36±1.07，SSIM 0.0533±0.0109，FID 76.99

**CPU基线：**
- 训练10 epoch后，噪声预测损失从 1.10 降至 0.05
- 验证集损失：0.153
